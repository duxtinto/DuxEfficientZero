...
class DataWorker(object):
    ...
    def __init__(self, rank, replay_buffer, storage, config, span_context: Context):
        ...
        self.service_tracer = self.tracer_provider.get_tracer(
            __name__,
        )

        self.span_context = extract(span_context)

        with self.service_tracer.start_as_current_span(
                "Initialize worker {}".format(rank),
    ...
    def run(self):
        ...
        with torch.no_grad():
            while True:
                ...
                with self.service_tracer.start_as_current_span("preparing play".format(trained_steps)):
                ...
                while not dones.all() and (step_counter <= self.config.max_moves):
                    ...
                    if start_training and (total_transitions / max_transitions) > (
                        ...
                        with self.service_tracer.start_as_current_span("pause recording experiences"):
                            time.sleep(1)
                            continue
