...
from __refactored__.storage.instrumented_replay_buffer import InstrumentedReplayBuffer
...
def _train(model, target_model, replay_buffer, shared_storage, batch_storage, config, summary_writer):
    ...
    with module_tracer.start_as_current_span("wait until the replay buffer contains enough transitions to start training"):
        replay_buffer_size = ray.get(replay_buffer.get_total_len())
        while replay_buffer_size < config.start_transitions:
            if replay_buffer_size % 50 == 0:
                with module_tracer.start_as_current_span("waiting for more transitions") as span:
                    span.set_attribute("replay_buffer_size", ray.get(replay_buffer.get_total_len()))
                    span.set_attribute("num_initial_transitions", config.start_transitions)
            time.sleep(1)
            replay_buffer_size = ray.get(replay_buffer.get_total_len())

    with module_tracer.start_as_current_span("train the model"):
...
def train(config, summary_writer, model_path=None):
    ...
    module_tracer = trace.get_tracer(__name__)
    with module_tracer.start_as_current_span("starting storage actors"):

        storage = SharedStorage.remote(model, target_model)
        storage.set_span_context.remote(training_context)

        # prepare the batch and mctc context storage
        batch_storage = QueueStorage(15, 20)
        mcts_storage = QueueStorage(18, 25)

        replay_buffer_tracer = trace.get_tracer(
            __name__,
            tracer_provider=(make_trace_provider("smartfighters-efficientZero-replay-buffer"))
        )

        with replay_buffer_tracer.start_as_current_span(
            "replay buffer lifecycle",
            context=extract(training_context)
        ):
            replay_buffer_context = {}
            inject(replay_buffer_context)
            replay_buffer = InstrumentedReplayBuffer(
                replay_buffer_context,
                ReplayBuffer.options(name="replay_buffer", lifetime="detached", get_if_exists=True).remote(config)
            )

    with module_tracer.start_as_current_span("running workers"):
        # other workers
        workers = []
        ...
        # self-play workers
        # self_play_workers_tracer = trace.get_tracer(
        #     __name__,
        #     tracer_provider=(make_trace_provider("smartfighters-efficientZero-replay-buffer"))
        # )

        with module_tracer.start_as_current_span(
            "self-play workers lifecycle",
            context=extract(training_context)
        ) as replay_buffer_span:
            self_play_context = {}
            inject(self_play_context)
            data_workers = [
                DataWorker.remote(self_play_context, rank, replay_buffer, storage, config) for rank in range(0, config.num_actors)
            ]
    ...
    # try:
        with module_tracer.start_as_current_span("training loop"):
            # training loop
