...
class DataWorker(object):
    ...
    def __init__(self, span_context: Context, rank, replay_buffer, storage, config):
        ...
        storage: Any
            The model storage
        """
        # pydevd_pycharm.settrace('192.168.1.127', port=5675, stdoutToServer=True, stderrToServer=True)

        self.storage = storage

        self.service_tracer = trace.get_tracer(
            __name__,
            tracer_provider=(make_trace_provider("smartfighters-efficientZero-selfplay-worker-{}".format(rank)))
        )

        with self.service_tracer.start_as_current_span(
                "worker {} life cycle".format(rank),
                context=extract(span_context)
        ) as service_span:
            self.service_span_context = service_span.get_span_context()
    ...
    def run(self):
        ctx = trace.set_span_in_context(NonRecordingSpan(self.service_span_context))

        # with self.service_tracer.start_as_current_span(
        #         "running self play",
        #         context=ctx
        # ) as span:
        #     span.add_event("self play worker {} started".format(self.rank))

        with self.service_tracer.start_as_current_span(
                "running self play",
                context=ctx
        ) as service_span:
            self.service_span = service_span
                ...
                while True:
                    ...
                    while not dones.all() and (step_counter <= self.config.max_moves):
                        ...
                        for i in range(env_nums):
                            deterministic = False
                            if start_training:
                                distributions, value, temperature, env = roots_distributions[i], roots_values[i], \
                                                                         _temperature[i], envs[i]
                            else:
